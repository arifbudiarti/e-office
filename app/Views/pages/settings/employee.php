...
<script type="text/javascript">
    ...
    function save() {
        var url;
        if (save_method == 'add') {
            url = "<?php echo site_url('Action/rubrik/1') ?>";
        } else {
            url = "<?php echo site_url('Action/rubrik/2') ?>";
        }
    ...
    function edit(id) {
        ...
        $.ajax({
            url: "<?php echo site_url('Data/getRubrik/') ?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data) {
                console.log(data);
                $('[name="id_md_rubrik"]').val(data.id_md_rubrik);
                $('[name="rubrik"]').val(data.rubrik);

                $('#modal_form').modal('show'); // show bootstrap modal when complete loaded
                $('.modal-title').text('Edit Rubrik'); // Set title to Bootstrap modal title
    ...
    function die(id) {
        if (confirm('Are you sure delete this data?')) {
            ...
            $.ajax({
                url: "<?php echo site_url('Action/rubrik/3/') ?>/" + id,
                type: "POST",
                dataType: "JSON",
                success: function(data) {
    ...
    function getTable() {
        ...
        tablexx = $('#tabel_serverside').DataTable({
            ...
            "order": [],
            "ajax": {
                url: "<?php echo base_url("Data/rubrik"); ?>", // json datasource
